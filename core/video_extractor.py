...
class VideoDigitExtractor:
    ...
    def preview_and_select_roi(self, video_path, enable_timer=True):
        """预览并让用户框选ROI（通过tkinter RoiSelector对话框）

        注意：推荐直接使用 ui.roi_selector.RoiSelector。
        此方法保留用于向后兼容。
        """
        try:
            import tkinter as tk
            ...
            root = tk.Tk()
            root.withdraw()
            selector = RoiSelector(root, video_path, enable_timer=enable_timer)
    ...
    def process_video(self, video_path, rois, start_frame, interval=0.25):
        """处理视频主逻辑（同步版本）"""
        cap = cv2.VideoCapture(video_path)
        fps = cap.get(cv2.CAP_PROP_FPS)
        ...
        print(f"开始处理视频，从第{start_frame}帧（约{start_frame/fps:.1f}秒）开始...")
